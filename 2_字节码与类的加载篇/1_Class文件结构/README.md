...
![image-20210202112206222](.\images\image-20210202112206222.png)

https://docs.oracle.com/javase/specs/index.html

所有的JVN全部遵守Java虚拟机规范，也就是说所有的JVN环境都是一样的，这样一来字节码文件可以在各种JVM上运行。
...
![中篇_第1章：JVM结构](.\images\中篇_第1章：JVM结构.jpg)
...
![image-20210202113023382](.\images\中篇_第1章：理解执行引擎.jpg)
...
![image-20210202171349007](.\images\image-20210202171349007.png)
...
![image-20210202172706252](.\images\image-20210202172706252.png)

- 如lI何解读供虚拟机解释执行的二进制字节码?

方式一:一个一个二进制的看。这里用到的是Notepad++,需要安装一个HEX-Editor插件，或者使用Binary Viewer

![image-20210202102208217](.\images\image-20210202102208217.png)



方式二:使用javap指令:jdk自带的反解析工具

方式三:I使用IDEA插件: jclasslib或jclasslib bytecode viewer客户端工具。(可视化更好)



![image-20210202104229780](.\images\image-20210202104229780.png)

![image-20210202104255300](.\images\image-20210202104255300.png)
...
**对应字节码文件：**
![image-20210202175840841](.\images\image-20210202175840841.png)
...
![image-20210202180511625](.\images\image-20210202180511625.png)

![image-20210203100239724](.\images\image-20210203100239724.png)
...
![image-20210202184914812](.\images\image-20210202184914812.png)

**这里整体给大家讲解一遍，这里需要注意听。否则大家以后也很难有动力，踏实认真的一个字节一个字节的解读一遍。**

### 1-魔数:Class文件的标志属

![image-20210202185026882](.\images\image-20210202185026882.png)
...
- 它们共同构成了class文件的格式版本号。譬如某个class文件的主版本号为M，副版本号为m，那么这个Class 文件的格式版本号就确定为M.m。
- 版本号和Java编译器的对应关系如下表:
...
![image-20210202191352031](.\images\image-20210202191352031.png)
...
![image-20210202191805407](.\images\image-20210202191805407.png)

- 在版本号之后，紧跟着的是常量池的数量，以及若干个常量池表项。
- 常量池中常量的数量是不固定的，所以在常量池的入口需要放置一项u2类型的无符号数，代表常量池容量计数值（constant_pool_count)。与Java中语言习惯不一样的是，这个容量计数是从1而不是0开始的。

![image-20210202192607865](.\images\image-20210202192607865.png)
...
![image-20210202192859707](.\images\image-20210202192859707.png)
...
![image-20210202195650123](.\images\image-20210202195650123.png)

![image-20210202200008394](.\images\image-20210202200008394.png)
...
![image-20210203101329119](.\images\image-20210203101329119.png)

**18种常量没有出现byte、short、char，boolean的原因**：他们编译之后都可以理解为Integer

**比如第一项0a，这是一个方法符号引用，可以通过字节码文件对照得出**

![image-20210203102002878](.\images\image-20210203102002878.png)
...
- 细节说明:
  ...
  - CONSTAT Fieldref_info、CONSTAIT_Rethodref_info和CONSTANT_InterfaceMethodref_info结构表示字段、方法和接口方法
  ...
  - CONSTANT_NameAndType_info结构用于表示字段或方法，但是和之前的3个结构不同,CONSTANT_AameAndType_info结构没有指明该字段或方法所属的类或接口。
  - CONSTANT_utf8_info用于表示字符常量的值
  - CONSTANT_MethodHandle_info结构用于表示方法句柄CONSTANT_MethodType_info结构表示方士类型
...
- 解析方式
  ...
  - 一个字节一个字节解析
    ...
    ![image-20210203102904078](.\images\image-20210203102904078.png)
...
![image-20210203113935691](.\images\image-20210203113935691.png)
...
![image-20210203140433737](.\images\image-20210203140433737.png)
...
![image-20210203141619561](.\images\image-20210203141619561.png)

![image-20210203142619385](.\images\image-20210203142619385.png)
...
![image-20210203144447675](.\images\image-20210203144447675.png)
...
![image-20210203145453762](.\images\image-20210203145453762.png)
...
![image-20210203151352390](.\images\image-20210203151352390.png)
...
![image-20210203160424506](.\images\image-20210203160424506.png)



② javap -protected JavapTest.class 显示受保护的/公共类和成员

![image-20210203160440658](.\images\image-20210203160440658.png)



③ javap -p(或-private)JavapTest.class 显示所有类和成员（大于private的）

![image-20210203160501889](.\images\image-20210203160501889.png)

④ javap -package JavapTest.class 显示程序包/受保护的/公共类和成员（默认）,非私有的信息

![image-20210203160516313](.\images\image-20210203160516313.png)

⑤ javap -sysinfo JavapTest.class 显示正在处理的类的系统信息（路径，大小，日期，MD5 散列，源文件名）

![image-20210203160529086](.\images\image-20210203160529086.png)

⑥ javap -constants JavapTest.class

- 与前面命令的主要区别，常量值会显示出来`COUNTS = 1;`

![image-20210203160541940](.\images\image-20210203160541940.png)
